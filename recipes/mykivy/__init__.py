...
class MyKivyRecipe(CythonRecipe):
    name = 'mykivy'

    depends = [('sdl2', 'pygame'), 'pyjnius', 'pango', 'fontconfig2']

    def cythonize_build(self, env, build_dir='.'):
        super(MyKivyRecipe, self).cythonize_build(env, build_dir=build_dir)
    ...
    def cythonize_file(self, env, build_dir, filename):
        ...
        if basename(filename) in do_not_cythonize:
            return
        super(MyKivyRecipe, self).cythonize_file(env, build_dir, filename)

    def get_recipe_env(self, arch):
        env = super(MyKivyRecipe, self).get_recipe_env(arch)
